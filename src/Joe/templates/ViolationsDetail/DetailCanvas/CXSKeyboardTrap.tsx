...
export type CXSKeyboardTrapDetailCanvasProps = CanvasElementProps & {
  scrollRef?: React.RefObject<HTMLDivElement>;
  detailInfo?: CXSKeyboardTrapDetail;
  snapshotImage?: string;
};

export const CXSKeyboardTrapDetailCanvas: React.FC<
  CXSKeyboardTrapDetailCanvasProps
> = ({ canvas, scrollRef, snapshotImage, detailInfo }) => {
  ...
  const initializeDraw = async () => {
    ...
    image.onload = async () => {
      ...
      setObjUuids(
        await Promise.all(
          detailInfo.output.map(({ index: vIndex, path_rectangle }) =>
            createBoundRectObject({
              canvas,
              rect: path_rectangle.rectangle,
              labelText: vIndex.toString(),
